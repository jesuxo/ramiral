...
class CwtransferenciasController extends Controller
{
    ...
    private function comprimirImagenTransferencia($rutaOrigen, $rutaDestino, $extension)
    {
        try {
            ...
            if ($anchoOriginal > $maxLado or $altoOriginal > $maxLado) {
            ...
            switch ($extension) {
                ...
                case 'jpeg':
                    ...
                    break;
                case 'png':
                    // Para PNG, compresión máxima sin pérdida de calidad visible
            ...
            if (file_exists($rutaDestino) and filesize($rutaDestino) > filesize($rutaOrigen)) {
                copy($rutaOrigen, $rutaDestino);
            }

        } catch (\Exception $e) {
            // Si falla la compresión, solo mover el archivo original
            copy($rutaOrigen, $rutaDestino);
        }
    }
    ...
    public function DescargarAgain(Request $request)
    {
        $idtran = $request->id ?? 0;
        $transf = Cwtransferencia::find($idtran);
